...
BOOST_AUTO_TEST_CASE(checking_empty_html)
{
	...
	BOOST_CHECK_EQUAL("<html><head><title>Title</title></"
	                  ...
	                  "menu><footer><p>Copyright</p></footer></body></html>",
	                  html_generated);
}
...
BOOST_AUTO_TEST_CASE(paragraph)
{
	...
	auto erased_html_sink = Si::Sink<char, Si::success>::erase(
	    Si::make_container_sink(html_generated));
	tags::p(tags::cl("someclass"), Si::html::text("Testing p tag"))
	    .generate(erased_html_sink);
	BOOST_CHECK_EQUAL("<p class=\"someclass\">Testing p tag</p>",
	                  html_generated);
}
...
BOOST_AUTO_TEST_CASE(div_box)
{
	...
	auto erased_html_sink = Si::Sink<char, Si::success>::erase(
	    Si::make_container_sink(html_generated));
	tags::div(tags::cl(""), Si::html::text("Testing div tag"))
	    .generate(erased_html_sink);
...
BOOST_AUTO_TEST_CASE(span)
{
	...
	auto erased_html_sink = Si::Sink<char, Si::success>::erase(
	    Si::make_container_sink(html_generated));
	tags::span(tags::cl(""), Si::html::text("Testing span tag"))
	    .generate(erased_html_sink);
	BOOST_CHECK_EQUAL("<span class=\"\">Testing span tag</span>",
	                  html_generated);
}
...
BOOST_AUTO_TEST_CASE(ul)
{
	...
	auto erased_html_sink = Si::Sink<char, Si::success>::erase(
	    Si::make_container_sink(html_generated));
	tags::ul(tags::li(Si::html::text("Test heading")))
	    .generate(erased_html_sink);
