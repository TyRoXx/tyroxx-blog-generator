...
/*
   ...
   ventura::relative_path("how-to-choose-an-integer-type-0.cpp")) +
                "One of the few valid use cases for the built-in types is
   ...
   major kind of integer types are the pointery ones: " +
                "`std::size_t`, `std::ptrdiff_t` and `std::uintptr_t`. They are
   used when dealing with memory on the current machine. " +
   ...
   difference can be negative. `uintptr_t` is for manipulating pointers on the
   bit "
                        "level. If you did not know about `uintptr_t` before
   reading this article, chances are you won't need this type any soon. " +
                "`size_t` and `ptrdiff_t` are two of the most important types in
   C++ you have to know and make use of." +
                " This is a very small function how it would be written by
   someone who does not waste a single thought on choosing the right type." +
                snippet_from_file(snippets_source_code,
   ventura::relative_path("how-to-choose-an-integer-type-2.cpp")) +
                "These are my thoughts when I read this function. I tried to
   mark the WTF moments so that you can calculate the WTFs per line metric as a
   take-home exercise if you like." +
                snippet_from_file(snippets_source_code,
   ventura::relative_path("how-to-choose-an-integer-type-3.cpp")) +
                "I only have to change one little thing to make all these WTFs
   go away:" +
                snippet_from_file(snippets_source_code,
   ventura::relative_path("how-to-choose-an-integer-type-4.cpp")) +
                "Much better, isn't it? Is it really too much to ask for using
