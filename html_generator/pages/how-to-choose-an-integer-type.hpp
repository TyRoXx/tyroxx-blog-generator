tags::h3(tags::link("#", "how-to-choose-an-integer-type",
    ...
    Si::html::tag("p",
                  text("Prefer portable types like ") +
                      inline_code("std::uint32_t") +
                      text(" over the types without an explicit range. Use ") +
                      inline_code("int") + text(", ") + inline_code("long") +
                      text(", ") + inline_code("long long") +
                      text(" only if you have a reason to use exactly these "
                           "and not the portable "
                           "ones. C++ is different from Java and C#: ") +
                      inline_code("int") +
                      text(" etc are not the same size and range on every "
                           "platform. This makes "
                           "them hard to use even for C++ experts. They are "
                           "neither necessary "
    ...
    Si::html::tag(
        ...
        text(
            ...
            text(" in this case. Using the wrong type leads to unportable code "
                 "and "
                 "misunderstandings.")) +
    snippet_from_file(
    ...
    tags::p(
        "One of the few valid use cases for the built-in types is overloading "
    ...
    Si::html::tag(
        "p",
        text("The other major kind of integer types are the pointery ones: ") +
            inline_code("std::size_t") + text(", ") +
            inline_code("std::ptrdiff_t") + text(" and ") +
            inline_code("std::uintptr_t") +
            text(". They are used when dealing with memory on the current "
                 "machine. ") +
            inline_code("int") + text(" cannot store the sizes of objects on "
                                      "most contemporary machines. ") +
            inline_code("size_t") + text(" can. Use ") + inline_code("size_t") +
            text(". ") + inline_code("ptrdiff_t") +
            text(" is for differences between memory addresses. It is signed "
                 "because a "
                 "difference can be negative. ") +
            inline_code("uintptr_t") +
            text(" is for manipulating pointers on the bit "
                 "level. If you did not know about ") +
            inline_code("uintptr_t") +
            text(" before reading this article, chances are "
                 "you won't need this type any soon. ") +
            inline_code("size_t") + text(" and ") + inline_code("ptrdiff_t") +
            text(" are two of the most important types in C++ you have to know "
                 "and "
                 "make use of.") +
            text(" This is a very small function how it would be written by "
                 "someone who "
                 "does not waste a single thought on choosing the right "
                 "type.")) +
    ...
    tags::p(
        "These are my thoughts when I read this function. I tried to mark the "
